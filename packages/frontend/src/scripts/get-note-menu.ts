...
import { clipsCache, followedChannelsCache } from '@/cache.js';
...
export function getNoteMenu(props: {
	...
	if ($i) {
		...
		...($i.isModerator || $i.isAdmin ? [
			...
			...(appearNote.userId === $i.id || appearNote.user.username === 'instance.actor' || $i.isModerator || $i.isAdmin ? [
				{ type: 'divider' },
				appearNote.userId === $i.id ? {
					icon: 'ti ti-edit',
...
export async function getRenoteMenu(props: {
	...
	const appearNote = isRenote ? props.note.renote as Misskey.entities.Note : props.note;
	const followedChannels = await followedChannelsCache.fetch();
	...
	if (followedChannels.length) {
		items = items.concat([{ type: 'divider' }, {
			...
			icon: 'ti ti-repeat',
			children: async () => followedChannels.map((channel) => {
				return {
					text: channel.name,
					icon: 'ti ti-device-tv',
					action: () => {
