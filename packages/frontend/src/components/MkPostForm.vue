...
>
	<header :class="$style.header">
		<div :class="$style.headerLeft">
			...
			<button ref="accountMenuEl" v-click-anime v-tooltip="i18n.ts.account" class="_button" @click="openAccountMenu">
				<img :class="$style.avatar" :src="(postAccount ?? $i).avatarUrl" :alt="(postAccount ?? $i).username" style="border-radius: 100%;"/>
			</button>
		</div>
		<div :class="$style.headerRight">
			<template v-if="!(targetChannel != null && fixed)">
				<button v-if="anonymouslySendToUser" class="_button" :class="[$style.headerRightItem, $style.visibility]" disabled>
					<span :class="$style.headerRightButtonText">
						<I18n :src="i18n.ts.anonymouslySendToUser" tag="span">
							<template #user>
								<MkA v-user-preview="anonymouslySendToUser.id" :to="userPage(anonymouslySendToUser)">
									<MkUserName :user="anonymouslySendToUser"/>
								</MkA>
							</template>
						</I18n>
					</span>
				</button>
				<button v-else-if="targetChannel == null" ref="visibilityButton" v-tooltip="i18n.ts.visibility" :class="['_button', $style.headerRightItem, $style.visibility]" @click="setVisibility">
			...
			<button v-if="visibility !== 'specified' && !anonymouslySendToUser" v-tooltip="i18n.ts._visibility.disableFederation" class="_button" :class="[$style.headerRightItem, { [$style.danger]: localOnly }]" :disabled="targetChannel != null" @click="toggleLocalOnly">
	...
	<MkInfo v-if="localOnly && !targetChannel && !anonymouslySendToUser" warn>{{ i18n.ts.disableFederationDeprecationWarning }}</MkInfo>
	...
	<div :class="[$style.textOuter, { [$style.withCw]: useCw }]">
		...
		<div v-if="visibility === 'followers'" :class="$style.followersBar"></div>
		<div v-if="visibility === 'specified'" :class="$style.specifiedBar"></div>
	...
	<footer ref="footerEl" :class="$style.footer">
		<div :class="$style.footerLeft">
			<button v-tooltip="i18n.ts.attachFile + ' (' + i18n.ts.upload + ')'" class="_button" :class="$style.footerButton" :disabled="isAnonymouslySending" @click="chooseFileFromPc"><i class="ti ti-photo-plus"></i></button>
			<button v-tooltip="i18n.ts.attachFile + ' (' + i18n.ts.fromDrive + ')'" class="_button" :class="$style.footerButton" :disabled="isAnonymouslySending" @click="chooseFileFromDrive"><i class="ti ti-cloud-download"></i></button>
			...
			<button v-tooltip="i18n.ts.publicLetter" class="_button" :class="$style.footerButton" :disabled="hasTargetChannel" @click="setASUT"><i class="ti" :class="anonymouslySendToUser ? 'ti-eyeglass-off' : 'ti-eyeglass'"></i></button>
		</div>
...
const anonymouslySendToUser = ref<Misskey.entities.UserDetailed | null>(props.initialAnonymouslySendToUser ?? null);
...
const isAnonymouslySending = computed((): boolean => anonymouslySendToUser.value !== null);
const hasTargetChannel = computed((): boolean => targetChannel.value != null);
...
const canPost = computed((): boolean => {
	return !props.mock && !posting.value && !posted.value && !uploader.uploading.value && (uploader.items.value.length === 0 || uploader.readyForUpload.value) &&
		...
		(!anonymouslySendToUser.value || (!anonymouslySendToUser.value.host && !$i.isSilenced && visibility.value === 'public' && !files.value.length && text.value.startsWith(`@${anonymouslySendToUser.value.username} `))) &&
...
watch([prefer.r.rememberNoteVisibility, store.r.visibility], ([rememberNoteVisibilityValue, visibilityValue]) => {
...
if (replyTargetNote.value && (replyTargetNote.value.user.username !== $i.username && !replyTargetNote.value.user.username.includes('.') || (replyTargetNote.value.user.host != null && replyTargetNote.value.user.host !== host))) {
...
function watchForDraft() {
	...
	watch(visibleUsers, () => saveDraft(), { deep: true });
	watch(anonymouslySendToUser, () => saveDraft(), { deep: true });
}
...
function clear() {
	...
	if (anonymouslySendToUser.value) {
		localOnly.value = props.initialLocalOnly ?? prefer.s.rememberNoteVisibility ? store.s.localOnly : prefer.s.defaultNoteLocalOnly;
...
function saveDraft() {
	...
	draftsData[draftKey.value] = {
		...
		data: {
			...
			anonymouslySendToUserId: anonymouslySendToUser.value?.id,
		},
	};
...
async function post(ev?: PointerEvent) {
	...
	cwInputEl.value?.blur();
	hashtagsInputEl.value?.blur();
	...
	if (withHashtags.value && hashtags.value && hashtags.value.trim() !== '') {
		const hashtags_ = hashtags.value.split(/\s+/).filter(x => x).map(x => x.startsWith('#') ? x : '#' + x).join(' ');
		if (!postData.text) {
	...
	misskeyApi('notes/create', postData, token).then((res) => {
		...
		nextTick(() => {
			...
			if (postData.anonymouslySendToUserId) {
				os.pageWindow(`/notes/${res.createdNote.id}`);
			}
		...
		posting.value = false;
		const errId = typeof err === 'object' && err !== null && 'id' in err ? String(err.id) : '';
		os.alert({
			type: 'error',
			text: err.message + '\n' + errId,
		});
...
function setASUT() {
	if (anonymouslySendToUser.value) {
		...
		os.selectUser().then(user => {
			const prefix = `@${Misskey.acct.toString(user)} `;
			...
			anonymouslySendToUser.value = user;
		});
	}
}
	...
	nextTick(() => {
		...
		if (!props.instant && !props.mention && !props.specified && !props.mock) {
			...
			if (draft != null) {
				...
				visibility.value = draft.data.id || !prefer.s.rememberNoteVisibility ? draft.data.visibility : store.s.visibility;
				...
				if (draft.data.anonymouslySendToUserId) {
					misskeyApi('users/show', { userId: draft.data.anonymouslySendToUserId }).then(res => {
						anonymouslySendToUser.value = res;
					});
				}
			}
		}
