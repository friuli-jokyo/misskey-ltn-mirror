	...
	<div class="_gaps_m">
		...
		<div v-if="totpLogin" class="2fa-signin" :class="{ securityKeys: user && user.securityKeys }">
			<div v-if="user && user.securityKeys" class="twofa-group tap-group">
				...
				<MkButton v-if="!queryingKey" type="button" @click="queryKey">
					{{ i18n.ts.retry }}
				</MkButton>
			</div>
			...
			<div class="twofa-group totp-group _gaps">
				...
				<MkInput v-if="user && user.twoFactorEnabled" v-model="token" type="text" :pattern="isBackupCode ? '^[A-Z0-9]{32}$' :'^[0-9]{6}$'" autocomplete="one-time-code" required :spellcheck="false" :inputmode="isBackupCode ? undefined : 'numeric'">
				...
				misskeyApi('signin', {
					...
					.then(({ handle, ...publicKey }) => {
						...
						webAuthnRequest(options)
							.then(async response => {
								signing.value = true;
								const credential = response.toJSON();
