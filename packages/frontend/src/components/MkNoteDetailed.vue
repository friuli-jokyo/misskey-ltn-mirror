...
>
	...
	<div v-if="isRenote" :class="$style.renote">
		...
		<span :class="$style.renoteText">
			<I18n :src="note.channel ? i18n.ts.renotedByChannel : i18n.ts.renotedBy" tag="span">
				<template #user>
	...
	<template v-else>
		<article :class="$style.note" @contextmenu.stop="onContextmenu">
			<header v-if="appearNote.anonymouslySendToUser" :class="$style.noteHeader">
			...
			<header v-else :class="$style.noteHeader">
				...
				<div :class="$style.noteHeaderBody">
					<div>
						...
						<span v-if="appearNote.user.isBot" :class="$style.isBot">{{ appearNote.user.username.includes('.') && appearNote.user.host === null ? 'system' : 'bot' }}</span>
					...
					<div v-if="!appearNote.anonymousChannelUsername" :class="$style.noteHeaderUsernameAndBadgeRoles">
						...
						<div v-if="appearNote.user.badgeRoles" :class="$style.noteHeaderBadgeRoles">
							<img v-for="(role, i) in appearNote.user.badgeRoles" :key="i" v-tooltip="role.name" :class="$style.noteHeaderBadgeRole" :src="role.iconUrl!" :alt="role.name"/>
						</div>
					</div>
			...
			<footer>
				...
				<button v-if="prefer.s.showTranslateButtonInNoteFooter && instance.translatorAvailable" class="_button" :class="$style.noteFooterButton" @mousedown="showTranslation ? showTranslation = false : translate()">
					<i v-if="translation" class="ti ti-note-off"></i>
