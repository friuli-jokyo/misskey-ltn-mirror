...
const MIGRATIONS = [
	{
		...
		async migrate() {
			...
			prefer.commit('showNoteVisibilityDial', store.s.showNoteVisibilityDial);
			...
			prefer.commit('showTranslateButtonInNoteFooter', store.s.showTranslateButtonInNoteFooter);
			prefer.commit('reactionsDisplaySize', store.s.reactionsDisplaySize);
			prefer.commit('adaptiveReactionsDisplaySize', store.s.adaptiveReactionsDisplaySize);
			prefer.commit('showReactedUserAvatars', store.s.showReactedUserAvatars);
			...
			prefer.commit('localNeighborNotes', store.s.localNeighborNotes);
			prefer.commit('globalNeighborNotes', store.s.globalNeighborNotes);
			prefer.commit('postFormOnChannelShouldPostToChannel', store.s.postFormOnChannelShouldPostToChannel);
			prefer.commit('autoOpenCws', store.s.autoOpenCws);
			prefer.commit('showTimelineReplies', store.s.showTimelineReplies);
	...
	{
		version: '2026.3.0-ltn.28323',
		async migrate() {
			const pluginsStr = miLocalStorage.getItem('miux:plugins');
			if (pluginsStr) {
				try {
					const plugins = JSON.parse(pluginsStr);
					if (Array.isArray(plugins) && plugins.length) {
						prefer.commit('plugins', plugins);
					}
				} catch (error) {
					console.error('Failed to parse miux:plugins:', error);
				}
			}
		},
	},
...
// TODO: そのうち消す
export function migrateOldSettings(lastVersion: string | null) {
	if (!lastVersion) {
		return;
	}
