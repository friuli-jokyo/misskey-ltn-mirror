...
<template>
	<div class="_gaps_s">
		...
		<XFolder v-if="matchQuery([i18n.ts._role._options.driveUploadBandwidth, 'driveUploadBandwidthDurationHrCapacityMbPairs'])" v-model:policyMeta="policyMetaModel.driveUploadBandwidthDurationHrCapacityMbPairs" :isBaseRole="isBaseRole" :readonly="readonly">
			<template #label>{{ i18n.ts._role._options.driveUploadBandwidth }}</template>
			<template #valueText>{{ JSON.stringify(valuesModel.driveUploadBandwidthDurationHrCapacityMbPairs) }}</template>
			<template #default="{ disabled }">
				<MkCodeEditor v-model="driveUploadBandwidthDurationHrCapacityMbPairsJson" lang="json5" :disabled="disabled"/>
			</template>
		</XFolder>
		...
		<XFolder v-if="matchQuery([i18n.ts._role._options.selfAssignability, 'selfAssignability'])" v-model:policyMeta="policyMetaModel.selfAssignability" :isBaseRole="isBaseRole" :readonly="readonly">
			<template #label>{{ i18n.ts._role._options.selfAssignability }}</template>
			<template #valueText>{{ JSON.stringify(valuesModel.selfAssignability) }}</template>
			<template #default="{ disabled }">
				<MkCodeEditor v-model="selfAssignabilityJson" lang="json5" :disabled="disabled"/>
			</template>
		</XFolder>
	</div>
</template>
...
const driveUploadBandwidthDurationHrCapacityMbPairsJson = computed({
	get: () => JSON5.stringify(valuesModel.value.driveUploadBandwidthDurationHrCapacityMbPairs, null, 2),
	set: (v: string) => { valuesModel.value.driveUploadBandwidthDurationHrCapacityMbPairs = JSON5.parse(v); },
});

const selfAssignabilityJson = computed({
	get: () => JSON5.stringify(valuesModel.value.selfAssignability, null, 2),
	set: (v: string) => { valuesModel.value.selfAssignability = JSON5.parse(v); },
});
