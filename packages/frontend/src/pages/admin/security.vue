		...
		<FormSuspense :p="init">
			<div class="_gaps_m">
				...
				<MkFolder>
					...
					<div class="_gaps_m">
						<MkTextarea v-model="bannedMediaDomains">
...
const bannedMediaDomains = ref<string>('');
...
async function init() {
	...
	bannedMediaDomains.value = meta.bannedMediaDomains?.join('\n') || '';
}
...
function save() {
	os.apiWithDialog('admin/update-meta', {
		...
		bannedMediaDomains: bannedMediaDomains.value.split('\n').filter(v => v),
	}).then(() => {
		fetchInstance(true);
	});
}
