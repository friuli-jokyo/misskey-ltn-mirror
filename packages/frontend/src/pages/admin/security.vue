	...
	<div class="_spacer" style="--MI_SPACER-w: 700px; --MI_SPACER-min: 16px; --MI_SPACER-max: 32px;">
		<SearchMarker path="/admin/security" :label="i18n.ts.security" :keywords="['security']" icon="ti ti-lock" :inlining="['botProtection']">
			<div class="_gaps_m">
				...
				<SearchMarker v-slot="slotProps" :keywords="['banned', 'media', 'domains', 'blacklist']">
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						<template #label><SearchLabel>Banned Media Domains</SearchLabel></template>
						<template v-if="bannedMediaDomainsForm.modified.value" #footer>
							<MkFormFooter :form="bannedMediaDomainsForm"/>
						</template>

						<div class="_gaps_m">
							<SearchMarker>
								<MkTextarea v-model="bannedMediaDomainsForm.state.bannedMediaDomains">
									<template #label><SearchLabel>Banned Media Domains List</SearchLabel></template>
								</MkTextarea>
							</SearchMarker>
						</div>
					</MkFolder>
...
const bannedMediaDomainsForm = useForm({
	bannedMediaDomains: meta.bannedMediaDomains?.join('\n') || '',
}, async (state) => {
	await os.apiWithDialog('admin/update-meta', {
		bannedMediaDomains: state.bannedMediaDomains.split('\n'),
	});
	fetchInstance(true);
});
