...
export function getNoteMenu(props: {
	...
	if ($i) {
		...
		if (!appearNote.user.username.includes('.') || appearNote.user.host !== null) {
			menuItems.push({
				type: 'parent',
				icon: 'ti ti-user',
				text: i18n.ts.user,
		...
		if (appearNote.userId === $i.id || appearNote.user.username.includes('.') && appearNote.user.host === null || $i.isModerator || $i.isAdmin) {
			menuItems.push({ type: 'divider' });
			if (appearNote.userId === $i.id) {
				menuItems.push({
					icon: 'ti ti-edit',
...
function renoteRipple(renoteButton: ShallowRef<HTMLElement | null | undefined>) {
