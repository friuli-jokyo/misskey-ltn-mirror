...
export async function postButtonTriggered(resolved: PathResolvedResult): Promise<void> {
	if (prefer.s.postFormOnChannelShouldPostToChannel && resolved.route.name === 'channel') {
		const channelId = resolved.props.get('channelId');
