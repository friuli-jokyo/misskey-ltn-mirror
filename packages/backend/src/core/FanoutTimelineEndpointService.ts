		...
		if (!shouldFallbackToDb) {
			...
			if (ps.me) {
				...
				filter = (note) => {
					...
					if (isUserRelated(note.renote, userIdsWhoBlockingMe, !!note.renote?.anonymousChannelUsername || ps.ignoreAuthorFromBlock)) return false;
					if (isUserRelated(note.renote, userIdsWhoMeMuting, !!note.renote?.anonymousChannelUsername || ps.ignoreAuthorFromMute)) return false;
